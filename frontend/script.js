...
async function openStudentPerformanceReportFromEncoded(encodedPayload, reportMode = 'subject') {
    ...
    const showSubject = subject => {
        ...
        subjectPrintButton?.addEventListener('click', (event) => {
            ...
            const subjectRows = rows.map(item => `<tr><th>${escReport(item.skill || '-')}</th><td>${escReport(item.learningOutcome || '-')}</td><td><span class="circle">○</span>${escReport(item.excellentDescription || '')}</td><td><span class="circle">○</span>${escReport(item.satisfactoryDescription || '')}</td><td><span class="circle">○</span>${escReport(item.needsPracticeDescription || '')}</td></tr>`).join('') || '<tr><td colspan="5">No performance record found.</td></tr>';
