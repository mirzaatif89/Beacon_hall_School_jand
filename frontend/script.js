...
function buildStudentFullPortfolioReport(student, records, esc) {
    ...
    const allSubjects = [...new Set([...standardSubjects, ...records.map(row => String(row.subject || '').trim()).filter(Boolean)])];
    ...
    const genericRows = (items) => items.map(label => `<tr><th>${esc(label)}</th><td></td><td></td><td></td><td></td></tr>`).join('');
